...
def test_get_key_unit_from_msg(wahapedia_svc):
    ...
    result = wahapedia_svc.getKeyUnitFromMsg(units, msg)
...
def test_get_key_unit_from_msg_gretchin(wahapedia_svc):
    # Leer tokens desde el archivo
    with open("resources/wh40k/wh40k_tokens.txt", "r", encoding="utf-8") as f:
        units = [line.strip() for line in f if line.strip()]
...
def test_get_url_base_unit(wahapedia_svc):
    # Test con una pregunta sobre Space Marines Rhino
...
def test_get_url_base_unit_no_faction_found(wahapedia_svc):
...
def test_classify_from_list(ai_models):
    ...
    result = ai_models.classify_from_list_pipeline(options, message)
    ...
    unit_result = ai_models.classify_from_list(units, unit_message)
...
def test_chat_endpoint_wahapedia(wahapedia_svc, verify_basic_response):
...
def test_classify_user_message_section(wahapedia_svc):
    casos = [
        # Space Marines
    ...
    for mensaje, esperado in casos:
        resultado = wahapedia_svc.classify_user_message_section(mensaje)
...
def test_chat_endpoint_wahapedia_armas(wahapedia_svc):
    """
...
def test_chat_endpoint_wahapedia_estratagemas(wahapedia_svc):
    """
...
def test_chat_endpoint_wahapedia_clasificacion_fallida(wahapedia_svc):
    """
